...
1. That *filtering* a dataset means *refining* data to:

   - Remove errors.
   - Ensure that features{{< footnote >}}Characteristics or attributes of data.{{< /footnote >}} are relevant to your question. 
   - Reduce noise and/or outliers.
   - Improve analytical efficiency.

2. The fact that filtering inevitably modifies the raw dataset. As such, it is important to either *filter* in code or store filtered data separately from raw data.
3. The importance of *documenting* all filtering operations (ideally, in a README.md).
4. Various methods for filtering, including **removing data by attribute value**, **detrending**, **smoothing**, and **isolating data by frequency**. 
5. *Resampling,* which refers to the process of generating one or more *new* data points from some sample{{< footnote >}}Here, *sample* refers to a subset (i.e., *statistical sample*) of some population.{{< /footnote >}}.
